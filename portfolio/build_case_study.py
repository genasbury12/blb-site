...
Matches the BLB brand per CLIENT-SITE-PLAYBOOK Phase 1: Fraunces + Karla
+ Ephesis, cream paper, deep pink #A93F63, gold accents. Run: python3 build_case_study.py"""
...
<link href="https://fonts.googleapis.com/css2?family=Fraunces:ital,opsz,wght@0,9..144,500;0,9..144,600;1,9..144,500&family=Karla:wght@400;700&family=Ephesis&family=Cormorant+Garamond:ital,wght@1,500&family=Parisienne&display=swap" rel="stylesheet">
<style>
:root{{--paper:#FBF6EE;--tint:#F4EBDE;--ink:#2E1F27;--pink:#A93F63;--purple:#7C2B48;
  --lilac:#F0DCE2;--gold:#BC8A47;--muted:#6E5C63;
  --display:'Fraunces',Georgia,serif;--body:'Karla',sans-serif;--script:'Ephesis',cursive;
...
h1,h2,h3{{font-family:var(--display);font-weight:500;line-height:1.1;letter-spacing:0}}
.eyebrow{{display:inline-flex;align-items:center;gap:10px;font-family:var(--display);font-weight:700;
  font-size:.8rem;letter-spacing:.24em;text-transform:uppercase;color:var(--purple);font-family:var(--body);font-weight:700}}
...
.mark{{background:linear-gradient(180deg,transparent 62%,#EBDCC3 62%)}}
.mark.pink{{background:linear-gradient(180deg,transparent 62%,var(--lilac) 62%)}}
...
.btn:hover{{transform:translateY(-3px);box-shadow:0 14px 30px rgba(46,31,39,.22)}}
.btn-pink{{background:var(--pink);border-color:var(--pink)}}
.btn-outline{{background:transparent;color:var(--ink)}}
.btn-outline:hover{{background:var(--ink);color:var(--paper)}}

nav{{position:sticky;top:0;z-index:50;background:rgba(251,246,238,.93);backdrop-filter:blur(10px);
  border-bottom:1px solid rgba(124,43,72,.14)}}
.nav-inner{{display:flex;align-items:center;justify-content:space-between;padding:16px 24px;max-width:1100px;margin:0 auto}}
.logo{{font-family:var(--display);font-weight:600;font-size:1.25rem}}
...
.hero{{padding:96px 0 80px;background:
  radial-gradient(800px 400px at 90% -10%,rgba(188,138,71,.14),transparent 70%),
  radial-gradient(700px 400px at -5% 100%,rgba(169,63,99,.10),transparent 70%),var(--paper)}}
...
.chip{{background:#fff;border:1.5px solid #E5D3B8;border-radius:50px;padding:9px 20px;
  font-family:var(--display);font-weight:600;font-size:.86rem;color:var(--muted)}}
.chip b{{color:var(--pink);font-family:var(--display)}}

.meta{{background:var(--ink);color:#F2E8DC;padding:34px 0}}
...
.meta .m b{{display:block;font-family:var(--display);font-size:.72rem;letter-spacing:.2em;
  text-transform:uppercase;color:#D9A9BC;margin-bottom:5px}}
...
.quotecard{{background:#fff;border:1.5px solid #E5D3B8;border-radius:22px;padding:34px 36px;
  box-shadow:0 22px 50px rgba(46,31,39,.10);transform:rotate(-1deg)}}
...
.browser{{background:#fff;border-radius:18px;overflow:hidden;border:1px solid rgba(124,43,72,.18);
  box-shadow:0 30px 70px rgba(46,31,39,.16)}}
.bbar{{display:flex;gap:6px;align-items:center;padding:12px 16px;background:var(--tint)}}
...
.shot{{background:#fff;border-radius:16px;overflow:hidden;border:1px solid rgba(124,43,72,.16);
...
.inc div{{display:flex;gap:12px;align-items:flex-start;padding:10px 0;border-bottom:1px dashed rgba(188,138,71,.45);font-size:.98rem}}
.inc div::before{{content:"✓";flex:none;width:24px;height:24px;border-radius:8px;background:var(--lilac);
  color:var(--pink);font-weight:700;display:flex;align-items:center;justify-content:center;font-size:.85rem}}

.steps{{background:var(--tint)}}
.steprow{{display:grid;grid-template-columns:repeat(5,1fr);gap:18px;margin-top:46px}}
.step{{background:#fff;border-radius:18px;padding:26px 22px;border:1px solid rgba(124,43,72,.15);
  box-shadow:0 14px 34px rgba(46,31,39,.07)}}
.step .n{{font-family:var(--script);font-size:1.9rem;color:var(--gold);line-height:1}}
...
.final .lede{{margin:0 auto 34px;color:#D8CDC2}}
footer{{background:var(--ink);color:#A9989F;text-align:center;font-size:.85rem;padding:26px 16px;
...
<section class="tour"><div class="wrap">
  ...
  <a href="{LIVE}" target="_blank" rel="noopener" style="color:var(--pink)">click through the real thing</a>.</p>
</div></section>
...
<section class="final"><div class="wrap">
  <span class="eyebrow" style="color:#D9A9BC">Your turn</span>
